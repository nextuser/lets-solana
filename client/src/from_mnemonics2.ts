...
dotenv.config()
// BIP44路径
const BIP44_SOLANA_PATH = "m/44'/501'/0'/0'/0'";
const BIP44_SUI_PATH = "m/44'/784'/0'/0'";
const BIP_PATH_FORMAT  =  "m/44'/%u'/0'/0'"
...
async function generate_keypair(mnemonic:string,coinType:string) :Keypair{
        ...
        return generate_keypair_path(mnemonic,bip44path);
}

async function generate_keypair_path(mnemonic:string,derivePath:string) :Keypair{
        // 生成种子
...
// 输出公钥和私钥
    ...
    let keypair2 = await generate_keypair_path(mnemonic,BIP44_SOLANA_PATH);
    console.log('SOLANA Public Key2:', keypair2.publicKey.toBase58());
    console.log('SOLANA Private Key2:', bs58.encode(keypair2.secretKey)); // Base64 编码私钥
}
